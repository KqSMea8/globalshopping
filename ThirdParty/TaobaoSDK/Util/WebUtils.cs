...
namespace Baichuan.Api.Util
{
    ...
    public sealed class WebUtils
    {
        ...
        public HttpWebRequest GetWebRequest(string url, string method, IDictionary<string, string> headerParams)
        {
            ...
            var defaultProxy = "http://47.88.192.166:3128";

            if (!string.IsNullOrEmpty(ConfigurationManager.AppSettings["BaichuanProxy"]))
            {
                defaultProxy = ConfigurationManager.AppSettings["BaichuanProxy"];
            }

            WebProxy webProxy = new WebProxy(defaultProxy);
            req.Proxy = webProxy;
