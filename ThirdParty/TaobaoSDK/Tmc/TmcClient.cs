...
using Taobao.Baichuan.Link;
using Taobao.Baichuan.Link.Channel;
using Taobao.Baichuan.Link.Endpoints;
using Baichuan.Api.Util;
using Taobao.Baichuan.Link.Util;
using Baichuan.Api;
...
namespace Baichuan.Tmc
{
    ...
    public class TmcClient
    {
        ...
        {
            ILoggerFactory loggerFactory = new ClientLoggerFactory(false, true, true, true, true);
            this.Log = loggerFactory.Create(this) as ClientLog;
            this._endpoint = new Endpoint(loggerFactory, this._id);
            this._endpoint.ChannelSelector = new ClientChannelSharedSelector(loggerFactory) { HeartbeatPeriod = this._heartbeatInterval };
        ...
        private void InternalOnMessage(object sender, EndpointContext context)
        {
            if (this.Log.IsDebugEnabled)
                this.Log.DebugFormat("messsage from {0}: {1}", context.MessageFrom, this.Dump(context.Message));
            ...
            ThreadPool.QueueUserWorkItem(o =>
            {
                ...
                {
                    if (this.Log.IsDebugEnabled)
                        this.Log.DebugFormat("process message error: {0}", args._reason);
                    return;
                }
                ...
                try
                {
                    this.Confirm(msg);
                    if (this.Log.IsDebugEnabled)
                        this.Log.DebugFormat("confirm message: {0}", this.Dump(context.Message));
                }
                catch (Exception e)
                {
                    if (this.Log.IsWarnEnabled)
                        this.Log.Warn(string.Format("confirm message error: {0}", this.Dump(context.Message)), e);
                }
            });
        }
        ...
        {
            if (this.Log.IsDebugEnabled)
                this.Log.DebugFormat("ack messsage from {0}: {1}", e.MessageFrom, e.Message);
        }
