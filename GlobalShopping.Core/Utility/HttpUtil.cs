        ...
        public HttpResult GetHtml(HttpItem item)
        {
            ...
            try
            {
                ...
                using (response = (HttpWebResponse)request.GetResponse())
                {
                    ...
                    result.Header["location"] = response.ResponseUri.ToString();
                    ...
                    //获取Byte
                    var ResponseByte = _stream.ToArray();
                    _stream.Close();
                    if (ResponseByte != null & ResponseByte.Length > 0)
                    {
                        ...
                        if (encoding == null)
                        {
                            var meta = Regex.Match(Encoding.Default.GetString(ResponseByte),
                                "<meta([^<]*)charset=([^<]*)[\"']", RegexOptions.IgnoreCase);
                            var c = (meta.Groups.Count > 1) ? meta.Groups[2].Value.ToLower().Trim() : string.Empty;
                            if (c.Length > 2)
                            {
                                try
                                {
